...
public class Write{
    ...
    public Write(String deviceName)
    {
    ...
    public void StartWriteEPC(String ip, String selected_tid, String epc) {
        ...
        try {
            ...
            Thread.sleep(1);
            TCPDataOut.write(hexStringToByteArray("7001020802000000")); // select tid bank
            System.out.println("Send TAGMSK_BANK command 7001020802000000");
            Thread.sleep(1);
            TCPDataOut.write(hexStringToByteArray("7001030800000000"));
            System.out.println("Send TAGMSK_PTR command 7001030800000000");
            Thread.sleep(1);
            TCPDataOut.write(hexStringToByteArray("7001040860000000"));
            System.out.println("Send TAGMSK_LEN command 7001040860000000");
            Thread.sleep(1);
            
            byte[] tidheader = hexStringToByteArray(selected_tid);
            
            TCPDataOut.write(hexStringToByteArray(String.format("70010508%02X%02X%02X%02X", tidheader[0], tidheader[1],tidheader[2], tidheader[3])));
            System.out.println("Send TAGMSK_0_3 command");
            Thread.sleep(1);
            TCPDataOut.write(hexStringToByteArray(String.format("70010608%02X%02X%02X%02X", tidheader[4], tidheader[5],tidheader[6], tidheader[7])));
            System.out.println("Send TAGMSK_4_7 command");
            Thread.sleep(1);
            TCPDataOut.write(hexStringToByteArray(String.format("70010708%02X%02X%02X%02X", tidheader[8], tidheader[9],tidheader[10], tidheader[11])));
            ...
            byte[] epcheader = hexStringToByteArray(epc);
            ...
            Thread.sleep(1);
            TCPDataOut.write(hexStringToByteArray(String.format("7001040a06000000"))); // number of words to write
            ...
            for (int i = 0; i < 6; i++)
            {
                TCPDataOut.write(hexStringToByteArray(String.format("7001%02X0a%02X%02X%02X00", 9+i, epcheader[i*2+1], epcheader[i*2], 2+i)));// data word to write with offset 2 (PC offset)
